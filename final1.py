...
def plot_next_three_days(selected_date, selected_company):
    ...
    # Set black background and adjust elements with red line
    plt.figure(figsize=(10, 5), facecolor='black')  # Set figure background to black
    ax = plt.gca()  # Get current axes

    # Set the plot (axes) background to black
    ax.set_facecolor('black')

    # Plot the data with a red line
    plt.plot(next_three_days_data.index, next_three_days_data['Close'], marker='o', linestyle='-', color='red')  

    # Customize title, labels, and grid
    plt.title(f'Stock Prices for {selected_company} for the Next Three Days Starting {selected_date.date()}', color='white')
    plt.xlabel('Date', color='white')
    plt.ylabel('Stock Price (Close)', color='white')

    # Customize ticks and grid for black background
...
# Plot actual vs predicted stock prices with black background
fig, ax = plt.subplots(figsize=(14, 6), facecolor='black')
ax.set_facecolor('black')
ax.plot(data.index[:len(train_predict)], y_train, label='Actual Train', color='cyan')
ax.plot(data.index[len(train_predict):(len(train_predict) + len(test_predict))], y_test, label='Actual Test', color='lime')
...
ax.set_title(f'Actual vs. Predicted Stock Prices for {company_symbol}', color='white')
ax.set_xlabel('Date', color='white')
ax.set_ylabel('Stock Price', color='white')

# Customize ticks and labels for dark background
...
# Plot future predictions with black background
st.write("Forecasted Stock Prices for the Next Two Quarters")
quarters_ahead = ['Q1', 'Q2']
fig2, ax2 = plt.subplots(figsize=(8, 4), facecolor='black')
ax2.set_facecolor('black')
ax2.bar(quarters_ahead, future_predictions.flatten(), color='lightcoral')
ax2.set_title(f'Forecasted Stock Prices for {company_symbol}', color='white')
ax2.set_xlabel('Quarter Ahead', color='white')
